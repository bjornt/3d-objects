...
//all distances in mm
n1 = 8; //red gear number of teeth. Set to zero if you don't want to print a pinion.
n5 = 24;  //gray rack number of teeth.  Set to 1 if you don't want to print a rack- that's as small as it gets (you'll get 2 teeth if you set it to zero... I haven't tried to determine why that happens)
mm_per_tooth = 2;
...
hole         = 1.6;   //hole diameter in pinion gear
height       = 6;  // distance from bottom of the rack to the tips of the teeth
...
translate([0, 0, height/2]) color([1.00,0.75,0.75]) gear(mm_per_tooth,n1,height,hole, pressure_angle=33
);

difference() {
    cylinder(h=2.5, r=3.2, $fn=64);
    translate([0, 0, -1]) cylinder(h=3.6, d=2, $fn=64);
}
}
